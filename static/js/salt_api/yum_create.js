			...
			{	
				...
				$.get("/exec_cmd",{"group_name":group_name}, function(ret){
					...
					$.each(ret,function(i,item){
						...
						console.log("输出IP",item.ip,ret[i].ip)
						//处理多选的联动
...
function saltFunc()
	...
	if (active_tab=='命令执行')//执行命令执行tab的按钮事件
	{
		...
		console.log("执行的命令为",order);
		var args_linux = $("#salt_args_linux").val();
		var args_win = $("#salt_args_win").val();
		var common_cmd = $("#common_cmd").find("option:selected").text()
		console.log("linux参数为:",args_linux);
		console.log("win参数为:",args_win);
		console.log("common_cmd:",common_cmd);
...
//					                        ]
				                    $.each(ret,function(i,item){
				                    	console.log("索引",i)
				                    	console.log("内容",item)
				                    	console.log("类型",typeof(item))
				                    	console.log("截取字符串",String(item).substring(0,5))
				                    	if(i!='msg' && i!='code') 
				                    	{
...
//				                    });
	...
	if (active_tab==='文件修改')
	{
		...
		console.log("修改的linux配置文件",edit_config_linux)
		console.log("修改的windows配置文件",edit_config_win)
		console.log("edit_config_pre",edit_config_pre)
		console.log("edit_config_last",edit_config_last)
